...
const AddProjectType = () => {
  ...
  const fetchProjectTypes = async () => {
    try {
      ...
      showToast("Failed to fetch project types", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure?')) {
      try {
        await productApi.deleteProjectType(id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        await productApi.updateProjectType(editingItem._id, formData);
        showToast('Project Type updated');
      } else {
        await productApi.createProjectType(formData);
  ...
  const filteredList = projectTypes.filter(item =>
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="bg-white rounded-lg shadow-sm mb-6 p-6 flex justify-between items-center">
        <h2 className="text-2xl font-bold text-blue-600">Project Types</h2>
        <button onClick={handleAddNew} className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-blue-700">
          <PlusCircle size={20} /> Add Project Type
        </button>
      </div>

      {showForm && (
        <div className="bg-white rounded-lg shadow-sm mb-6 p-6 animate-fadeIn border border-blue-100">
          <h5 className="font-semibold text-lg mb-4">{editingItem ? 'Edit Project Type' : 'Add New Project Type'}</h5>
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        ...
        {loading ? (
          ...
          <div className="text-center py-8 text-gray-500">No project types found.</div>
        ) : (
