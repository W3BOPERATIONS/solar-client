...
export default function SetPrice() {
  ...
  // Margin modal data (Keeping static for now as it seems to be a calculator or setting not fully detailed in backend reqs yet, but can be dynamic later)
  ...
  // Purchase history data (Mock for now, as backend doesn't have it yet)
  ...
  const fetchPrices = async () => {
    ...
    try {
      ...
      const processedData = data.map(item => {
        const isRed = item.marketPrice > item.benchmarkPrice;
        const isGreen = item.marketPrice <= item.benchmarkPrice;

        return {
          ...item,
          // Mapping backend fields to frontend expected fields for display if names differ, though they seem similar
  ...
  const toggleRowEdit = async (id) => {
    ...
    setTableData(prev => prev.map(item => {
      if (item.id === id) {
        ...
        // Recalculate background if saving
        const isRed = item.marketPrice > item.benchmarkPrice;
        const isGreen = item.marketPrice <= item.benchmarkPrice;
  ...
  const calculateSummary = (data) => {
    ...
    data.forEach(item => {
      ...
      // Update highest margin (Using GST as proxy from original code, or calculate real margin)
  ...
  const handleSetPrice = () => {
    // Recalculate Logic only for visual confirmation, as data is saved individually
  ...
  const handleResetFilters = () => {
    ...
    fetchPrices(); // Reload original data
    setShowSummary(false);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      ...
      <div className="mb-4">
        <div className="bg-white p-3 rounded-lg shadow-sm">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <h3 className="text-xl font-bold text-gray-900 mb-0">Set Price for Solarkit</h3>
          ...
          <div className="bg-white rounded-lg shadow-sm">
            ...
            <div className="p-4">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                {/* Other filters can be added similarly based on data availability */}
              </div>
            </div>
          </div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-800 text-white">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-center font-semibold">Benchmark (Per Kw)</th>
                    <th className="px-4 py-3 text-center font-semibold">Market Price (per Kw)</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {loading ? (
                    <tr><td colSpan="12" className="text-center py-4">Loading prices...</td></tr>
                  ) : tableData.length === 0 ? (
                    <tr><td colSpan="12" className="text-center py-4">No data found. Add new price.</td></tr>
                  ) : (
                    tableData.map((row) => (
                      <tr key={row.id} style={{ backgroundColor: row.backgroundColor }}>
                        ...
                        <td className="px-4 py-3 text-center min-w-[120px]">
                          <input
                            ...
                            onChange={(e) => handleTableInputChange(row.id, 'benchmarkPrice', parseInt(e.target.value) || 0)}
                        ...
                        <td className="px-4 py-3 text-center min-w-[120px]">
                          <input
                            ...
                            onChange={(e) => handleTableInputChange(row.id, 'marketPrice', parseInt(e.target.value) || 0)}
                            disabled={!row.isEditing}
                          />
                        </td>
                        ...
                        <td className="px-4 py-3 text-center min-w-[120px]">
                          <input
                            type="number"
                            className={`w-full px-2 py-1 border rounded text-center ${row.isEditing ? 'border-blue-500' : 'border-gray-300'}`}
                            value={row.gst}
            ...
            <div className="bg-white rounded-lg shadow-sm mt-4">
              ...
              <div className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                  {/* ... Summary Content ... */}
                  {/* Kept simple for brevity, logic is in calculateSummary */}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-auto">
            <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-gray-100">
              <h3 className="text-lg font-bold text-gray-800">Add New Price</h3>
            ...
            <div className="p-6">
              <form onSubmit={handleAddNewSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700">Benchmark Price</label>
                  <input type="number" className="w-full border rounded p-2" value={newPriceForm.benchmarkPrice} onChange={e => setNewPriceForm({ ...newPriceForm, benchmarkPrice: Number(e.target.value) })} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Market Price</label>
                  <input type="number" className="w-full border rounded p-2" value={newPriceForm.marketPrice} onChange={e => setNewPriceForm({ ...newPriceForm, marketPrice: Number(e.target.value) })} />
                </div>
                <div>
      ...
      {/* Purchase History Modal and Margin Modal can remain as placeholders or removed if not needed for dynamic part yet */}
    </div>
  );
}