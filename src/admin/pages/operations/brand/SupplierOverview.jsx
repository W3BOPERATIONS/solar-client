...
const BrandSupplierOverview = () => {
  ...
  // Filtered sub-categories driven by selected categories in the Filter Section
  ...
  const [modalClusters, setModalClusters] = useState([]);
  ...
  const fetchInitialData = async () => {
    ...
    try {
      const [
        ...
        getAllSuppliers(),
        getAllManufacturers(),
        locationApi.getCountries({ silent: true }),
        locationApi.getStatesHierarchy({ silent: true }),
        getProducts(),
        getCategories(),
        getSubCategories(),
        getProjectTypes(),
  ...
  useEffect(() => {
    const fetchFilterStates = async () => {
      if (selectedCountries.size === 0) {
        // Don't clear if we want the modal to work. 
        // But dashboard filters expect empty states if no country is selected?
        // Let's only fetch if there ARE selected countries.
        return;
      }
  ...
  useEffect(() => {
    const fetchFilterCities = async () => {
      ...
      const promises = Array.from(selectedDistricts).map(districtId =>
        axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/locations/cities?districtId=${districtId}&isActive=true`)
      );
      try {
        const responses = await Promise.all(promises);
  ...
  // Re-fetch sub-categories every time the categories filter changes
  useEffect(() => {
    const loadFilteredSubCategories = async () => {
      if (categories.length === 0) {
        // No category selected — show all sub-categories
        setFilteredSubCategories(masterSubCategories);
        return;
      }
      try {
        // Fetch sub-categories for every selected category in parallel
        const selectedCatIds = masterCategories
          .filter(c => categories.includes(c.name))
          .map(c => c._id);

        const results = await Promise.all(
          selectedCatIds.map(catId => getSubCategories(catId))
        );

        // Merge + deduplicate by _id
        const allSubs = results.flatMap(r => r.data || []);
        const unique = Array.from(
          new Map(allSubs.map(s => [s._id, s])).values()
        );
        setFilteredSubCategories(unique);

        // Reset any sub-category selections that are no longer valid
        const validNames = unique.map(s => s.name);
        setSubCategories(prev => prev.filter(sc => validNames.includes(sc)));
      } catch (err) {
        console.error('Error loading filtered sub-categories:', err);
      }
    };
    loadFilteredSubCategories();
  }, [categories, masterCategories, masterSubCategories]);
  ...
  const filteredSuppliers = suppliers.filter(supplier => {
    ...
    if (selectedCities.size > 0 && !selectedCities.has(supplier.city?._id || supplier.city)) return false;
  ...
  const handleEdit = (supplier) => {
    ...
    const clusterId = supplier.cluster?._id || supplier.cluster;
    const districtId = supplier.district?._id || supplier.district;
  ...
  const fetchModalClusters = async (stateId) => {
    if (!stateId) { setModalClusters([]); return; }
    try {
      const data = await locationApi.getClustersHierarchy(stateId, { silent: true });
      setModalClusters(Array.isArray(data) ? data : (data?.data || []));
    } catch (e) { console.error(e); }
  };

  const fetchModalDistricts = async (clusterId) => {
    if (!clusterId) { setModalDistricts([]); return; }
    try {
      const data = await locationApi.getDistrictsHierarchy(clusterId, { silent: true });
      setModalDistricts(Array.isArray(data) ? data : (data?.data || []));
    } catch (e) { console.error(e); }
  };

  const fetchModalCities = async (districtId) => {
    if (!districtId) { setModalCities([]); return; }
    try {
      const data = await locationApi.getCitiesHierarchy(districtId, { silent: true });
      setModalCities(Array.isArray(data) ? data : (data?.data || []));
    } catch (e) { console.error(e); }
  };

  const handleModalStateChange = (e) => {
    const val = e.target.value;
    setModalForm({ ...modalForm, state: val, cluster: '', district: '', city: '' });
    setModalClusters([]); setModalDistricts([]); setModalCities([]);
    fetchModalClusters(val);
  };

  const handleModalClusterChange = (e) => {
    const val = e.target.value;
    setModalForm({ ...modalForm, cluster: val, district: '', city: '' });
    setModalDistricts([]); setModalCities([]);
    ...
    if (catName) {
      ...
      if (selCat) {
        try {
          ...
          setModalSubCategories(Array.isArray(res) ? res : (res.data || []));
        } catch (err) { console.error(err); }
      }
    }
  };
  ...
  const handleModalProjectTypeChange = async (e) => {
    ...
    if (ptName) {
      ...
      try {
        ...
        setModalSubProjectTypes(Array.isArray(res) ? res : (res.data || []));
      } catch (err) { console.error(err); }
    }
  };
  ...
  return (
    <div className="container-fluid p-6 bg-gray-50 min-h-screen">
      ...
      {selectedDistricts.size > 0 && (
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6 border border-gray-200">
          <h2 className="text-lg font-semibold mb-4 border-l-4 border-blue-500 pl-3">Select Cities</h2>
          <div className="flex gap-2 mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {allCities.map((city) => (
              <div
                key={city._id}
                className={`border rounded-lg p-4 text-center cursor-pointer transition-all duration-200 ${selectedCities.has(city._id) ? 'border-orange-600 bg-orange-600 text-white' : 'border-gray-200'}`}
            ...
            return (
              <>
                ...
                <FilterDropdown
                  ...
                  options={filteredSubCategories.map(sc => sc.name).filter(Boolean).sort()}
            ...
            <form onSubmit={handleModalSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 border-t pt-4">
                ...
                <div>
                  ...
                  <select className="w-full border p-2 rounded" value={modalForm.cluster} onChange={handleModalClusterChange} required disabled={!modalForm.state}>
                    <option value="">Select Cluster</option>
                    {modalClusters.map(c => <option key={c._id} value={c._id}>{c.name}</option>)}
                  </select>
                </div>
                <div>
